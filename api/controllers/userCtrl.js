...
const registerController = async (req, res) => {
  const session = await userModel.startSession(); // Start a transaction
  session.startTransaction();

  try {
    const { email, password, isDoctor, name, specialization } = req.body;
    ...
    // If the user is a doctor, create a doctor entry
    if (isDoctor) {
...
const bookAppointmentController = async (req, res) => {
  try {
    ...
    // Get doctor userId from doctorId
    const doctor = await doctorModel.find({ userId: req.body.doctorId });
    console.log("doaaaaaaa", doctor, req.body.doctorId);
    if (!doctor) {
    ...
    const doctorUser = await userModel.find({ userId: req.body.doctorId });
    if (!doctorUser) {
      return res.status(404).send({
        success: false,
